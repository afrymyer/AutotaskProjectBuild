  ...
  return (
    <section>
      <div className="page-header">
        <div>
          <h1>Team availability calendar</h1>
          <p className="muted">
            Engineers are counted as <strong>free</strong> on a day when their
            scheduled load is below 50% (under 4h of an 8h day). Click a day for the breakdown.
          </p>
        </div>
        <div className="month-nav">
          <button onClick={() => setMonthOffset((m) => m - 1)}>‹ Prev</button>
          <div className="month-label">{monthLabel}</div>
          <button onClick={() => setMonthOffset((m) => m + 1)}>Next ›</button>
          {monthOffset !== 0 && (
...
function DayButton({
  ...
  return (
    <button
      ...
      style={{ borderTop: `4px solid var(--util-${color})` }}
      onClick={onClick}
    >
      ...
      <ul>
        {rows.map(({ resource, hours }) => (
          <li key={resource.autotask_id}>
            ...
            <span className="muted">{Math.round(hours * 10) / 10}h</span>
          </li>
        ))}
      </ul>
    </div>
...
function Legend() {
  return (
    <div className="legend">
      <span><i style={{ background: 'var(--util-green)' }} /> ≥75% of team free</span>
